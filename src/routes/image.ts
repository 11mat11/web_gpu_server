...
const InputModeSchema = z
	.enum(['random', 'custom'])
	.describe('Źródło danych wejściowych obrazu.')
	.example('random');
...
function decodeBase64Image(inputBase64: string, expectedBytes: number): Uint8Array {
	...
	if (buffer.byteLength !== expectedBytes) {
		throw new Error(
			`Invalid input size. Expected ${expectedBytes} bytes, got ${buffer.byteLength}.`,
		);
	}
		...
		async (req, reply) => {
			...
			try {
				if (body.backend === 'webgpu') {
					const result = await gaussianBlurWebGpu(body.width, body.height, inputPacked, {
						readback: true,
					});
					if (!result.output) {
