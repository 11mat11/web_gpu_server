...
const BackendSchema = z
	...
	.describe('Backend obliczeń macierzy.')
	.example('webgpu');
const InputModeSchema = z
	.enum(['random', 'custom'])
	.describe('Źródło danych wejściowych macierzy.')
	.example('random');
...
const MatrixBodySchema = z
	.object({
		size: z.number().int().min(1).default(512).describe('Rozmiar macierzy NxN.').example(512),
		backend: BackendSchema.default('webgpu')
			.describe('Backend wykonania mnożenia.')
			.example('webgpu'),
		inputMode: InputModeSchema.default('random')
			.describe('Tryb wejścia: random generuje A/B, custom używa danych z requestu.')
			.example('random'),
		optimized: z
			.boolean()
			.default(false)
			.describe('Dla GPU: true=tiled/shared memory, false=naive.')
			.example(false),
		randomMin: z.number().default(0).describe('Dolna granica losowania (random mode).').example(0),
		randomMax: z.number().default(9).describe('Górna granica losowania (random mode).').example(9),
		randomSeed: z
			.number()
			.int()
			.min(0)
			.max(U32_MAX)
			.optional()
			.describe('Opcjonalny seed deterministyczny (random mode).')
			.example(123456),
		matrixA: z
			.array(z.array(z.number().finite()))
			.optional()
			.describe('Macierz A w trybie custom (size x size).')
			.example([
				[1, 2],
				[3, 4],
			]),
		matrixB: z
			.array(z.array(z.number().finite()))
			.optional()
			.describe('Macierz B w trybie custom (size x size).')
			.example([
				[5, 6],
				[7, 8],
			]),
	})
	.describe('Parametry mnożenia macierzy do benchmarku WebGPU/CUDA.')
	.example({
		size: 512,
		backend: 'webgpu',
		inputMode: 'random',
...
function multiplySquareMatricesCpu(
	size: number,
...
function exceedsGpuMatrixLimits(gpu: GPUDevice, size: number): boolean {
	...
	return (
		matrixBytes > gpu.limits.maxBufferSize || matrixBytes > gpu.limits.maxStorageBufferBindingSize
	);
}
...
export async function matrixRoute(server: FastifyInstance) {
	server.post(
				...
				body: {
					...
					properties: {
						...
						backend: {
							...
							default: 'webgpu',
							description:
								'Silnik obliczeń: webgpu (WGSL), cuda (N-API addon), cpu (fallback/referencja).',
						},
						inputMode: {
							type: 'string',
							enum: ['random', 'custom'],
							default: 'random',
							description:
								'random = generacja A/B (GPU dla webgpu/cuda, CPU dla cpu), custom = użyj matrixA i matrixB z requestu.',
						},
						optimized: {
							type: 'boolean',
							default: false,
							description:
								'Dotyczy webgpu/cuda: true = tiled/shared memory, false = naive. Dla cpu parametr jest ignorowany.',
						},
						randomMin: {
							type: 'number',
							default: 0,
							description: 'Dolna granica losowania, używana tylko gdy inputMode=random.',
						},
						randomMax: {
							type: 'number',
							default: 9,
							description: 'Górna granica losowania, używana tylko gdy inputMode=random.',
						},
						randomSeed: {
							type: 'number',
							minimum: 0,
						...
						matrixA: {
							...
							description:
								'Wymagane tylko dla inputMode=custom. Format number[][] o rozmiarze size x size.',
						},
						matrixB: {
							type: 'array',
							items: {
							...
							description:
								'Wymagane tylko dla inputMode=custom. Format number[][], ten sam rozmiar co matrixA.',
						},
					},
				},
				response: {
					200: {
						...
						properties: {
							...
							optimized: {
								type: 'boolean',
								description:
									'Finalnie użyta ścieżka: true = tiled GPU, false = naive lub backend cpu.',
							},
							serverDurationMs: { type: 'number' },
							generationDurationMs: {
								type: 'number',
								nullable: true,
								description: 'Czas generowania danych wejściowych (random mode).',
							},
							multiplyDurationMs: {
								type: 'number',
								nullable: true,
								description: 'Czas samego mnożenia macierzy.',
							},
							gpuDurationMs: {
								type: 'number',
								nullable: true,
								description:
									'Suma generationDurationMs + multiplyDurationMs (czas GPU/CPU dla obliczeń).',
							},
							backendDurationMs: {
								type: 'number',
								description:
									'Czas wykonania backendu (end-to-end w serwerze) bez narzutów sieciowych.',
							},
							timingSource: {
								type: 'string',
		...
		async (req, reply) => {
			...
			try {
				if (body.backend === 'webgpu') {
					...
					if (tooLargeForGpu) {
						...
						const result = await multiplyRandomSquareMatricesWebGpu(
							body.size,
							body.randomMin,
							body.randomMax,
							{
			...
			const hasGpuDuration = generationDurationMs !== null || multiplyDurationMs !== null;
			gpuDurationMs = hasGpuDuration
				? (generationDurationMs ?? 0) + (multiplyDurationMs ?? 0)
				: null;
			...
			const hostBytes = sumByteLengths(matrixA, matrixB, matrixC);
			const memory = {
				gpuBytes,
				hostBytes,
			...
			const response = {
				...
				generationDurationMs:
					generationDurationMs === null ? null : Number(generationDurationMs.toFixed(3)),
				multiplyDurationMs:
					multiplyDurationMs === null ? null : Number(multiplyDurationMs.toFixed(3)),
				gpuDurationMs: gpuDurationMs === null ? null : Number(gpuDurationMs.toFixed(3)),
				backendDurationMs: Number(computeDurationMs.toFixed(3)),
				timingSource,
				memory,
				gflops: Number(gflops.toFixed(2)),
				...(matrixC && body.size <= MATRIX_C_RESPONSE_LIMIT
					? { matrixC: toMatrix2D(matrixC, body.size) }
					: {}),
			};
