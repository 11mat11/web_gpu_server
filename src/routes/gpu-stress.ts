...
function getStressTouchShaderSource(): Promise<string> {
	if (!stressTouchShaderSourcePromise) {
		stressTouchShaderSourcePromise = readFile(STRESS_SHADER_PATH, 'utf8').catch((err) => {
...
async function allocateVram(device: GPUDevice, targetBytes: number): Promise<GPUBuffer[]> {
	...
	while (allocated < targetBytes) {
		...
		try {
			...
			console.log(
				`[GPU Stress]  chunk ${buffers.length}: +${Math.round(chunkBytes / 1024 / 1024)} MB  (total ${Math.round(allocated / 1024 / 1024)} MB)`,
			);
		} catch (err) {
			if (buf) {
				try {
...
async function touchAllocatedBuffers(device: GPUDevice, buffers: GPUBuffer[]): Promise<void> {
	...
	try {
		...
		for (let i = 0; i < buffers.length; i++) {
			...
			const params = new Uint32Array([stepWords, totalWords, ((i + 1) * 2654435761) >>> 0, 0]);
				...
				body: {
					...
					properties: {
						durationSec: {
							type: 'number',
							default: 30,
							description: 'Seconds to hold memory (1–300)',
						},
						targetMb: {
							type: 'number',
							default: DEFAULT_MB,
							minimum: 256,
							maximum: 24576,
							description: 'Explicit MB to allocate (default 1024)',
						},
					},
				},
				response: {
		...
		async (req, reply) => {
			...
			if (buffers.length === 0) {
				for (const fallbackMb of [512, 256]) {
					if (buffers.length === 0) {
						console.log(
							`[GPU Stress] ⚠️  Requested ${targetMb} MB failed, trying fallback ${fallbackMb} MB`,
						);
			...
			if (buffers.length > 0) {
				console.log(
					`[GPU Stress] ✅ Holding ${actualMb} MB in ${buffers.length} buffer(s) for ${body.durationSec}s (WGSL touch complete)`,
				);
			} else {
		...
		'/:id',
		{
			schema: {
				...
				response: {
					200: {
						type: 'object',
						properties: { released: { type: 'boolean' }, id: { type: 'string' } },
					},
					400: errorResponseSchema,
