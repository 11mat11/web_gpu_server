...
const renderShaderCode = readFileSync(
	new URL('./shaders/renderSceneRender.wgsl', import.meta.url),
	'utf8',
);
const computeShaderCode = readFileSync(
	new URL('./shaders/renderSceneCompute.wgsl', import.meta.url),
	'utf8',
);
...
function validateShapes(shapes: Float32Array, count: number): void {
	...
	if (shapes.length !== expected) {
		throw new Error(
			`Shape buffer length mismatch. Expected ${expected} floats, got ${shapes.length}.`,
		);
	}
}
...
export async function renderSceneWebGpuCompute(
	...
	try {
		...
		device.queue.writeBuffer(
			paramsBuffer,
			0,
			new Uint32Array([width, height, Math.max(0, Math.floor(count)), 0]),
		);
