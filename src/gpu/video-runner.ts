...
const histogramShaderCode = readFileSync(
	new URL('../gpu/shaders/histogram.wgsl', import.meta.url),
	'utf8',
);
...
function resolveTargetSize(quality: Exclude<VideoQuality, '1080p'>): {
	width: number;
	height: number;
	bytes: number;
} {
...
function createStorageBuffer(device: GPUDevice, size: number, copySrc = false): GPUBuffer {
	return device.createBuffer({
		size,
		usage:
			GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST | (copySrc ? GPUBufferUsage.COPY_SRC : 0),
	});
}
...
export async function processVideoFrameWebGpu(
	...
	const supportsTimestamp = pipeline.device.features.has('timestamp-query');
...
export async function computeHistogramWebGpu(frameRgba: Uint8Array): Promise<VideoHistogramResult> {
	...
	try {
		...
		const waits: Promise<void>[] = [
			...
			histogramReadback.mapAsync(GPUMapMode.READ),
		];
		if (queryReadback) {
			waits.push(queryReadback.mapAsync(GPUMapMode.READ));
		}
