...
export async function loadMlpModelToWebGpu(weights: Float32Array): Promise<LoadedWebGpuMlpModel> {
  if (weights.length !== TOTAL_WEIGHT_COUNT) {
    throw new Error(`Invalid MLP weights length. Expected ${TOTAL_WEIGHT_COUNT}, got ${weights.length}.`)
  }
  ...
  for (const [name, bytes] of [
    ...
    if (bytes > maxStorage) {
      throw new Error(`Buffer ${name} exceeds maxStorageBufferBindingSize (${bytes} > ${maxStorage}).`)
    }
  }
