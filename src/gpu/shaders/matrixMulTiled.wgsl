...
fn main(@builtin(workgroup_id) wid: vec3<u32>, @builtin(local_invocation_id) lid: vec3<u32>) {
	...
	for (var t: u32 = 0u; t < tiles; t++) {
		...
		tileA[localRow][localCol + 1u] =
			select(0.0, matA[row * size + aCol1], row < size && aCol1 < size);
		tileA[localRow + 1u][localCol] =
			select(0.0, matA[row1 * size + aCol], row1 < size && aCol < size);
		tileA[localRow + 1u][localCol + 1u] =
			select(0.0, matA[row1 * size + aCol1], row1 < size && aCol1 < size);

		tileB[localRow][localCol] = select(0.0, matB[bRow * size + col], bRow < size && col < size);
		tileB[localRow][localCol + 1u] =
			select(0.0, matB[bRow * size + col1], bRow < size && col1 < size);
		tileB[localRow + 1u][localCol] =
			select(0.0, matB[bRow1 * size + col], bRow1 < size && col < size);
		tileB[localRow + 1u][localCol + 1u] =
			select(0.0, matB[bRow1 * size + col1], bRow1 < size && col1 < size);
