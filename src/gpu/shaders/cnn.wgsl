...
fn conv2dRelu(@builtin(global_invocation_id) gid: vec3<u32>) {
	let outX = gid.x;
	let outY = gid.y;
	...
	for (var inCh: u32 = 0u; inCh < inChannels; inCh = inCh + 1u) {
		for (var ky: u32 = 0u; ky < 3u; ky = ky + 1u) {
			let inY = i32(outY) + i32(ky) - 1;
			if (inY < 0 || inY >= i32(inHeight)) {
				continue;
			}

			for (var kx: u32 = 0u; kx < 3u; kx = kx + 1u) {
				let inX = i32(outX) + i32(kx) - 1;
				if (inX < 0 || inX >= i32(inWidth)) {
					continue;
				}
...
@compute @workgroup_size(8, 8, 1)
fn maxPool2x2(@builtin(global_invocation_id) gid: vec3<u32>) {
	let outX = gid.x;
	let outY = gid.y;
	let channel = gid.z;
