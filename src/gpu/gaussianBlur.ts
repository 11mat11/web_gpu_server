...
const gaussianBlurShaderSource = readFileSync(new URL('./shaders/gaussianBlur.wgsl', import.meta.url), 'utf8')
...
export async function gaussianBlurWebGpu(
  ...
  try {
    ...
    horizontalPass.dispatchWorkgroups(Math.ceil(width / WORKGROUP_SIZE), Math.ceil(height / WORKGROUP_SIZE), 1)
    horizontalPass.end()
    ...
    verticalPass.dispatchWorkgroups(Math.ceil(width / WORKGROUP_SIZE), Math.ceil(height / WORKGROUP_SIZE), 1)
    verticalPass.end()
