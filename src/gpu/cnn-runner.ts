...
function createStorageBuffer(
	...
	return device.createBuffer({
		...
		usage:
			...
			(includeCopySrc ? GPUBufferUsage.COPY_SRC : 0),
	});
}
...
export async function loadCnnModelToWebGpu(weights: Float32Array): Promise<LoadedWebGpuCnnModel> {
	if (weights.length !== TOTAL_WEIGHT_COUNT) {
		throw new Error(
			`Invalid CNN weights length. Expected ${TOTAL_WEIGHT_COUNT}, got ${weights.length}.`,
		);
	}
	...
	const inputBytes =
		CNN_INPUT_CHANNELS * CNN_INPUT_HEIGHT * CNN_INPUT_WIDTH * Float32Array.BYTES_PER_ELEMENT;
	const conv1OutBytes =
		CNN_CONV1_OUT_CHANNELS * CNN_INPUT_HEIGHT * CNN_INPUT_WIDTH * Float32Array.BYTES_PER_ELEMENT;
	const pool1OutBytes =
		CNN_CONV1_OUT_CHANNELS * CNN_POOL1_HEIGHT * CNN_POOL1_WIDTH * Float32Array.BYTES_PER_ELEMENT;
	const conv2OutBytes =
		CNN_CONV2_OUT_CHANNELS * CNN_POOL1_HEIGHT * CNN_POOL1_WIDTH * Float32Array.BYTES_PER_ELEMENT;
	const pool2OutBytes =
		CNN_CONV2_OUT_CHANNELS * CNN_POOL2_HEIGHT * CNN_POOL2_WIDTH * Float32Array.BYTES_PER_ELEMENT;
	const conv3OutBytes =
		CNN_CONV3_OUT_CHANNELS * CNN_POOL2_HEIGHT * CNN_POOL2_WIDTH * Float32Array.BYTES_PER_ELEMENT;
	const pool3OutBytes =
		CNN_CONV3_OUT_CHANNELS * CNN_POOL3_HEIGHT * CNN_POOL3_WIDTH * Float32Array.BYTES_PER_ELEMENT;
	const conv4OutBytes =
		CNN_CONV4_OUT_CHANNELS * CNN_POOL3_HEIGHT * CNN_POOL3_WIDTH * Float32Array.BYTES_PER_ELEMENT;
	const pool4OutBytes =
		CNN_CONV4_OUT_CHANNELS * CNN_POOL4_HEIGHT * CNN_POOL4_WIDTH * Float32Array.BYTES_PER_ELEMENT;
...
export async function predictWithWebGpuCnn(
	...
	const pool1Params = createParamsBuffer(
		device,
		'cnn-pool1-params',
		makeParams(CNN_CONV1_OUT_CHANNELS, CNN_INPUT_HEIGHT, CNN_INPUT_WIDTH),
	);
	...
	const pool2Params = createParamsBuffer(
		device,
		'cnn-pool2-params',
		makeParams(CNN_CONV2_OUT_CHANNELS, CNN_POOL1_HEIGHT, CNN_POOL1_WIDTH),
	);
	...
	const pool3Params = createParamsBuffer(
		device,
		'cnn-pool3-params',
		makeParams(CNN_CONV3_OUT_CHANNELS, CNN_POOL2_HEIGHT, CNN_POOL2_WIDTH),
	);
	const conv4Params = createParamsBuffer(
		...
		makeParams(
			CNN_CONV3_OUT_CHANNELS,
			CNN_CONV4_OUT_CHANNELS,
			CNN_POOL3_HEIGHT,
			CNN_POOL3_WIDTH,
			1,
		),
	);
	const pool4Params = createParamsBuffer(
		device,
		'cnn-pool4-params',
		makeParams(CNN_CONV4_OUT_CHANNELS, CNN_POOL3_HEIGHT, CNN_POOL3_WIDTH),
	);
	const dense1Params = createParamsBuffer(
		device,
		'cnn-dense1-params',
		makeParams(CNN_FLATTEN_SIZE, CNN_DENSE1_OUT, 1),
	);
	const dense2Params = createParamsBuffer(
		device,
		'cnn-dense2-params',
		makeParams(CNN_DENSE1_OUT, CNN_OUTPUT_SIZE, 0),
	);
	...
	try {
		...
		pass0.dispatchWorkgroups(
			Math.ceil(CNN_INPUT_WIDTH / 8),
			Math.ceil(CNN_INPUT_HEIGHT / 8),
			CNN_CONV1_OUT_CHANNELS,
		);
		pass0.end();
		...
		pass1.dispatchWorkgroups(
			Math.ceil(CNN_POOL1_WIDTH / 8),
			Math.ceil(CNN_POOL1_HEIGHT / 8),
			CNN_CONV1_OUT_CHANNELS,
		);
		pass1.end();
		...
		const pass3 = encoder.beginComputePass(
			querySet
				? { timestampWrites: { querySet, beginningOfPassWriteIndex: 6, endOfPassWriteIndex: 7 } }
				: {},
		);
		pass3.setPipeline(pipelines.pool);
		pass3.setBindGroup(0, pool2Bind);
		pass3.dispatchWorkgroups(
			Math.ceil(CNN_POOL2_WIDTH / 8),
			Math.ceil(CNN_POOL2_HEIGHT / 8),
			CNN_CONV2_OUT_CHANNELS,
		);
		pass3.end();
		...
		pass5.dispatchWorkgroups(
			Math.ceil(CNN_POOL3_WIDTH / 8),
			Math.ceil(CNN_POOL3_HEIGHT / 8),
			CNN_CONV3_OUT_CHANNELS,
		);
		pass5.end();
		...
		const pass6 = encoder.beginComputePass(
			querySet
				? { timestampWrites: { querySet, beginningOfPassWriteIndex: 12, endOfPassWriteIndex: 13 } }
				: {},
		);
		pass6.setPipeline(pipelines.conv);
		pass6.setBindGroup(0, conv4Bind);
		pass6.dispatchWorkgroups(
			Math.ceil(CNN_POOL3_WIDTH / 8),
			Math.ceil(CNN_POOL3_HEIGHT / 8),
			CNN_CONV4_OUT_CHANNELS,
		);
		...
		const pass7 = encoder.beginComputePass(
			querySet
				? { timestampWrites: { querySet, beginningOfPassWriteIndex: 14, endOfPassWriteIndex: 15 } }
				: {},
		);
		pass7.setPipeline(pipelines.pool);
		pass7.setBindGroup(0, pool4Bind);
		pass7.dispatchWorkgroups(
			Math.ceil(CNN_POOL4_WIDTH / 8),
			Math.ceil(CNN_POOL4_HEIGHT / 8),
			CNN_CONV4_OUT_CHANNELS,
		);
		pass7.end();

		const pass8 = encoder.beginComputePass(
			querySet
				? { timestampWrites: { querySet, beginningOfPassWriteIndex: 16, endOfPassWriteIndex: 17 } }
				: {},
		);
		...
		const pass9 = encoder.beginComputePass(
			querySet
				? { timestampWrites: { querySet, beginningOfPassWriteIndex: 18, endOfPassWriteIndex: 19 } }
				: {},
		);
...
export function unloadWebGpuCnnModel(model: LoadedWebGpuCnnModel): void {
	model.conv1W.destroy();
	model.conv1B.destroy();
	model.conv2W.destroy();
	model.conv2B.destroy();
	model.conv3W.destroy();
	model.conv3B.destroy();
	model.conv4W.destroy();
	model.conv4B.destroy();
	model.dense1W.destroy();
	model.dense1B.destroy();
	model.dense2W.destroy();
	model.dense2B.destroy();
	model.input.destroy();
	model.conv1Out.destroy();
	model.pool1Out.destroy();
	model.conv2Out.destroy();
	model.pool2Out.destroy();
	model.conv3Out.destroy();
	model.pool3Out.destroy();
	model.conv4Out.destroy();
	model.pool4Out.destroy();
	model.dense1Out.destroy();
	model.output.destroy();
}
