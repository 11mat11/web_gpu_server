...
const matrixMulShaderSource = readFileSync(
	new URL('./shaders/matrixMul.wgsl', import.meta.url),
	'utf8',
);
const matrixMulTiledShaderSource = readFileSync(
	new URL('./shaders/matrixMulTiled.wgsl', import.meta.url),
	'utf8',
);
const randomFillShaderSource = readFileSync(
	new URL('./shaders/randomFill.wgsl', import.meta.url),
	'utf8',
);
...
function createRandomParamsData(
	size: number,
	seed: number,
	minValue: number,
	maxValue: number,
...
export async function multiplySquareMatricesWebGpu(
	...
	const pipeline = options.optimized
		? getMatrixMulTiledPipeline(device)
		: getMatrixMulPipeline(device);
...
export async function multiplyRandomSquareMatricesWebGpu(
	...
	const mulPipeline = options.optimized
		? getMatrixMulTiledPipeline(device)
		: getMatrixMulPipeline(device);
	...
	const [fillDispatchX, fillDispatchY, fillDispatchZ] = computeDispatchGrid(
		totalFillWorkgroups,
	...
	try {
		...
		const hasValidGenerationGpuTimestamp =
			generationTimestampMs !== null && generationTimestampMs > 0;
		...
		const multiplyDurationMs = hasValidMultiplyGpuTimestamp
			? multiplyTimestampMs
