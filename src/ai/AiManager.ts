...
export class AiManager {
	...
	private buildSingleModelmemory(model: AiModel): AiSingleModelmemory {
		const hostAllocatedBytes =
			model === 'mlp' ? this.mlpHostWeightsBytes : this.cnnHostWeightsBytes;
		...
		const totalGpuAllocatedBytes =
			(webgpu?.gpuAllocatedBytes ?? 0) + (cuda?.gpuAllocatedBytes ?? 0);
		...
		return {
			...
			cuda: {
				...
				...(loadedBackends.includes('cuda')
					...
					: {
							reason:
								this.cudaUnavailableReason ??
								(cudaRuntime.enabled ? undefined : cudaRuntime.reason),
						}),
			},
		};
	}
	...
	private async readModelWeights(model: AiModel): Promise<Float32Array> {
		...
		const expectedFloats =
			model === 'mlp' ? mlpLayout.totalWeightCount : cnnLayout.totalWeightCount;
	...
	async loadModel(options?: AiLifecycleOptions): Promise<AiLoadResult> {
		...
		try {
			for (const model of models) {
				const needsWebgpuLoad =
					targets.includes('webgpu') && !this.getLoadedBackends(model).includes('webgpu');
				const needsCudaLoad =
					targets.includes('cuda') && !this.getLoadedBackends(model).includes('cuda');
				...
				if (needsWebgpuLoad) {
					try {
						...
						loadErrors.push(
							`${model}: ${error instanceof Error ? error.message : 'WebGPU load failed.'}`,
						);
					}
				}
			...
			if (this.getLoadedModels().length === 0) {
				throw new AiManagerError(
					loadErrors[0] ?? 'No backend is loaded.',
					'backend_unavailable',
					400,
				);
			}
	...
	async predictMlp(backend: AiBackend, input: Float32Array): Promise<AiPredictResult> {
		if (input.length !== mlpLayout.inputSize) {
			throw new AiManagerError(
				`Invalid input length. Expected ${mlpLayout.inputSize} float values.`,
				'invalid_input',
				400,
			);
		}
		...
		try {
			...
			throw new AiManagerError(
				error instanceof Error ? error.message : 'MLP prediction failed.',
				'ai_predict_failed',
				500,
			);
		}
	...
	async predictCnn(backend: AiBackend, input: Float32Array): Promise<AiCnnPredictResult> {
		...
		if (input.length !== expectedInput) {
			throw new AiManagerError(
				`Invalid input length. Expected ${expectedInput} float values.`,
				'invalid_input',
				400,
			);
		}
		...
		try {
			...
			throw new AiManagerError(
				error instanceof Error ? error.message : 'CNN prediction failed.',
				'ai_predict_failed',
				500,
			);
		}
